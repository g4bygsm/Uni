    ...
    public partial class Form1 : Form
    {
        ...
        private void PartitioneazaMonoton()
        {
            ...
            foreach (int i in varfuriSeparare)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    if (varfuri[j].Y < varfuri[i].Y) 
                    {
                        if (EsteDiagonalaValida(i, j))
                        {
                            if (varfuri[j].Y > maxY)
                            {
                                maxY = varfuri[j].Y;
        ...
        private bool PunctInPoligon(PointF p)
        {
            bool inauntru = false;
            for (int i = 0, j = varfuri.Count - 1; i < varfuri.Count; j = i++)
            {
                if (((varfuri[i].Y > p.Y) != (varfuri[j].Y > p.Y)) &&
                    (p.X < (varfuri[j].X - varfuri[i].X) * (p.Y - varfuri[i].Y) / (varfuri[j].Y - varfuri[i].Y) + varfuri[i].X))
                {
                    inauntru = !inauntru;
                }
            }
        ...
        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (desenInchis)
            {
                Pen penDiagonale = new Pen(Color.Purple, 2) { DashStyle = System.Drawing.Drawing2D.DashStyle.Dash };
                foreach (var diag in diagonaleMonotone)
                {
                    g.DrawLine(penDiagonale, varfuri[diag.Item1], varfuri[diag.Item2]);
                }
